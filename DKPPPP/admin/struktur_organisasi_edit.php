...
// Mengambil data berdasarkan ID yang akan diedit
$sql = "SELECT * FROM struktur_organisasi WHERE id='$id'";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  // Jika user upload gambar baru, lakukan proses upload
  if (!empty($_FILES['gambar']['name'])) {
    ...
    if ($uploadOk == 0) {
      ...
      if (move_uploaded_file($_FILES["gambar"]["tmp_name"], $target_file)) {
        ...
        $sql = "UPDATE struktur_organisasi SET nama_gambar='$nama_gambar', path_gambar='$target_file' WHERE id='$id'";
    ...
    $sql = "UPDATE struktur_organisasi SET nama_gambar='$nama_gambar' WHERE id='$id'";
    if ($conn->query($sql) === TRUE) {
      header("Location: manajemen_struktur_organisasi.php");
    } else {
      echo "Error: " . $sql . "<br>" . $conn->error;
            ...
            <form method="post" enctype="multipart/form-data">
              <div class="form-group">
                ...
                <select name="tipe" class="form-control" required>
                  ...
                  <option value="Struktur" <?= $data['tipe'] == "Struktur" ? "selected" : "" ?>>Struktur</option>
                  <option value="Fungsi" <?= $data['tipe'] == "Fungsi" ? "selected" : "" ?>>Fungsi</option>
                </select>
              </div>
              <div class="form-group">
                <label>Nama Gambar</label>
                <input type="text" name="nama_gambar" class="form-control" value="<?= htmlspecialchars($data['nama_gambar']); ?>" required>
              </div>
              <div class="form-group">
                <label>Ganti Gambar</label>
                <input type="file" name="gambar" class="form-control">
                <p>Gambar saat ini: <img src="<?= $data['path_gambar']; ?>" width="100" alt="gambar saat ini"></p>
              </div>
