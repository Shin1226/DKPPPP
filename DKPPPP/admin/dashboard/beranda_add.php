...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($uploadOk == 1) {
        if (move_uploaded_file($_FILES["gambar"]["tmp_name"], $target_file)) {
            ...
            $sql = "INSERT INTO beranda (judul, deskripsi, gambar, status, urutan) VALUES (?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssi", $judul, $deskripsi, $gambar, $status, $urutan);

            if ($stmt->execute()) {
                echo "Data berhasil disimpan.";
                header("Location: manajemen_beranda.php"); // Redirect ke halaman list setelah berhasil menambah data
                exit();
            } else {
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                <label>Urutan</label>
                                <input type="number" name="urutan" class="form-control" placeholder="Isi Urutan" required>
                            </div>
