...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Mendapatkan teks alternatif
    $alt_text = $_POST['alt_text'];

    // Memeriksa dan mengunggah gambar
if (isset($_FILES['logo_image']) && $_FILES['logo_image']['error'] === UPLOAD_ERR_OK) {
    ...
    if (move_uploaded_file($file_tmp, $target_file)) {
        ...
        $sql = "INSERT INTO client (logo_image_url) VALUES (?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s", $logo_image_url);
        $stmt->execute();

        if ($stmt->affected_rows > 0) {
            echo "<script>alert('Client berhasil ditambahkan!'); window.location.href='manajemen_client.php';</script>";
        } else {
            echo "<script>alert('Gagal menambahkan client!');</script>";
        }
    } else {
